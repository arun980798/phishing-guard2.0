...
// Runs on every page.  Collects DOM signals, sends them to
// the background worker, then shows a small popup notification.
// ============================================================

(function () {
  "use strict";

  // Prevent running twice on the same page (e.g. dynamic SPA nav)
  if (window.__phishGuardRan) return;
  window.__phishGuardRan = true;

  // ── 1. Collect DOM signals ────────────────────────────────
  function collectDOMSignals() {
    ...
    // Sensitive-data inputs (email, tel, text fields with sensitive names)
    const sensitiveNames = /user|email|mail|phone|mobile|card|credit|debit|cvv|pin|ssn|dob|birth/i;
    ...
    const sensitiveTextPattern = /verify your (account|identity|email|password|card)|enter your (password|credentials|social security|credit card|otp)|your account (has been|will be) (suspended|locked|limited)/i;
    ...
    if (
      ...
      document.body.getAttribute("oncontextmenu").includes("return false")
    ) {
  ...
  function showPopup(score, verdict, reasons) {
    ...
    const colorMap = {
      ...
      suspicious: { bg: "#d97706", border: "#b45309", icon: "⚠", label: "Suspicious" },
      phishing: { bg: "#dc2626", border: "#b91c1c", icon: "✖", label: "Phishing Risk" }
    };
